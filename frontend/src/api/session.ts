...
export async function getSession(
  ...
  const getCredentialRequestFromExtension = await new Promise(
    (resolve, reject) => {
      try {
        session.listen(async (credentialRequest) => {
          resolve(credentialRequest)
        })
        session.send(get_terms_response.data)
      } catch (e) {
        reject(e)
      }
    }
  ...
  const encryptedMessage = getCredentialRequestFromExtension

  console.log(encryptedMessage)

  const attestation_message = await client.post(
    credentialUrl,
