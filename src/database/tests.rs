...
use crate::database::dto::{Credential, Pagination};
use crate::database::querys::{
    construct_query, delete_attestation_request, get_attestation_request_by_id,
...
async fn check_attestation_requests_can_be_deleted(db_executor: PgPool) {
    ...
    let attesatation = insert_attestation_request(&default_credential, &db_executor)
        .await
...
async fn test_get_attestation_requests_with_data(db_executor: PgPool) {
    ...
    let second_attestattion = insert_attestation_request(&default_credential, &db_executor)
        .await
        .expect("Attestation creation should not fail.");
